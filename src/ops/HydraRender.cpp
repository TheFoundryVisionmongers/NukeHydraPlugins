...
#include "pxr/imaging/hdSt/resourceFactoryMetal.h"
#include "pxr/imaging/hdSt/resourceFactoryGL.h"
#endif
...
HydraRender::renderStripe(ImagePlane& plane)
{
    if (_needRender) {
        if (GeoOp* geoOp = op_cast<GeoOp*>(Op::input(0))) {
            sceneDelegate()->SyncFromGeoOp(nullptr, geoOp);
        }
        else {
...
#if PXR_METAL_SUPPORT_ENABLED
#if FN_USE_METAL_STORM
    HdStResourceFactoryInterface *resourceFactory = new HdStResourceFactoryMetal();
#else
    HdStResourceFactoryInterface *resourceFactory = new HdStResourceFactoryGL();
#endif
  GarchResourceFactory::GetInstance().SetResourceFactory(dynamic_cast<GarchResourceFactoryInterface*>(resourceFactory));
