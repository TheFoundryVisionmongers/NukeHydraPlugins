...
HydraRectLight::HydraRectLight(Node* node)
    : HydraLightOp(node, HdPrimTypeTokens->rectLight)
    , _textureFile("")
{
}
...
HydraRectLight::MakeLightKnobs(Knob_Callback f)
{
    ...
    SetRange(f, 0.05, 2);
#if PXR_VERSION >= 2105
    RegisterLightParamKnob(f, UsdLuxTokens->inputsWidth);
#else
...
#if PXR_VERSION >= 2105
    RegisterLightParamKnob(f, UsdLuxTokens->inputsHeight);
#else
...
#if PXR_VERSION >= 2105
    RegisterLightParamKnob(f, UsdLuxTokens->inputsTextureFile);
#else
